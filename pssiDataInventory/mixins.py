...
class PSSIDataInventoryBasicMixin(LoginRequiredMixin, UserPassesTestMixin):
    def test_func(self):
        return True
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not user_test_result and self.request.user.is_authenticated:
            return HttpResponseRedirect('/accounts/denied/?app=pssiDataInventory')
...
class AdminRequiredMixin(PSSIDataInventoryBasicMixin):
    def test_func(self):
        return is_admin(self.request.user)


class NationalAdminRequiredMixin(PSSIDataInventoryBasicMixin):
    def test_func(self):
        return is_nat_admin(self.request.user)


class RegionalAdminRequiredMixin(PSSIDataInventoryBasicMixin):
    def test_func(self):
        return is_regional_admin(self.request.user)


class SuperuserOrAdminRequiredMixin(PSSIDataInventoryBasicMixin):
    def test_func(self):
        return self.request.user.is_superuser or is_nat_admin(self.request.user)


class CanModifyRequiredMixin(PSSIDataInventoryBasicMixin):
