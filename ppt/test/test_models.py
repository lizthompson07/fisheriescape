...
class TestProjectModel(CommonTest):
    ...
    def test_12m_section(self):
        # a `project` that is attached to a given `section` should be accessible by the reverse name `ppt`
        section = FactoryFloor.SectionFactory()
        my_instance = self.instance
        my_instance.section = section
        my_instance.save()
        self.assertIn(my_instance, section.ppt.all())

    @tag('Project', 'models', '12m')
    def test_12m_functional_group(self):
        # a `project` that is attached to a given `functional_group` should be accessible by the reverse name `ppt`
    ...
    def test_12m_default_funding_source(self):
        # a `project` that is attached to a given `default_funding_source` should be accessible by the reverse name `ppt`
