...
var app = new Vue({
  ...
  methods: {
    ...
    getFilterData() {
      ...
      apiService(`/api/ppt/tags/${query}`).then(response => this.tags = response)
      apiService(`/api/ppt/funding-sources/${query}`).then(response => this.fundingSources = response)
            ...
            .then(response => {
              if (response.results) {
                if (response.next) {
                  ...
                  let endpoint2 = `/api/ppt/fte-breakdown/?year=${this.filter_fiscal_year}&ids=${pyIds}`;
