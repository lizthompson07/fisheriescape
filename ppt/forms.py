...
class UpcomingDateForm(forms.ModelForm):
    class Meta:
        ...
        fields = "__all__"
        widgets = {
            "date": forms.DateTimeInput(attrs=dict(type="datetime-local"), format="%Y-%m-%dT%H:%M:%S")
        }
...
class ReportSearchForm(forms.Form):
    REPORT_CHOICES = (
        ...
        (7, "HR Project-Position Allocation (csv)"),
        (8, "Capital Request Costs (csv)"),
        (8, "Project Equipment summary (csv)"),
    )
    report = forms.ChoiceField(required=True, choices=REPORT_CHOICES)
    ...
    def __init__(self, *args, **kwargs):
        ...
        section_choices = [(obj.id, obj.full_name) for obj in shared_models.Section.objects.filter(ppt__isnull=False).distinct()]
...
class PPTAdminUserForm(forms.ModelForm):
    class Meta:
...
PPTAdminUserFormset = modelformset_factory(
    model=models.PPTAdminUser,
