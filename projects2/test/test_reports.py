...
class ExportProjectSummaryReportTest(CommonTest):
    ...
    def setUp(self):
        self.create_projects(self.EXPECTED_N_YEAR_2020, [2020])
        self.create_projects(self.EXPECTED_N_YEAR_2021, [2021])
        self.create_projects(self.EXPECTED_N_YEAR_2020_2021, [2020, 2021])

    def create_projects(self, count, year):
        ...
        for p in projects:
            ...
            p.save()
            factory.ProjectYearFactory.create(project=p, start_date=datetime.datetime(year=year[0], month=1, day=1, tzinfo=timezone.get_current_timezone()))
