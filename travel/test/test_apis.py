...
class TestTripRequestAPIViewSet(CommonTest):
    def setUp(self):
        ...
        self.test_list_url = reverse("request-list", args=None)
        self.test_detail_url = reverse("request-detail", args=[self.instance.pk])

    @tag("api", 'request')
    def test_url(self):
        self.assert_correct_url("request-list", test_url_args=None, expected_url_path=f"/api/travel/requests/")
        self.assert_correct_url("request-detail", test_url_args=[self.instance.pk], expected_url_path=f"/api/travel/requests/{self.instance.pk}/")
