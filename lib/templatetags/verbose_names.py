...
def get_field_value(instance, field_name, format=None, display_time=False, hyperlink=None, nullmark="---", date_format="%Y-%m-%d",
    ...
    #  next, let's see if it is field in another table that we are trying to access (e.g. user.first_name
        ...
        try:
            ...
            if val is not None and val != "":
                ...
                elif hasattr(field_instance, "choices") and field_instance.choices and len(field_instance.choices) > 0:
    ...
    try:
        if field_value is None or field_value == "" or field_value == "None":
            field_value = nullmark
        elif field_value != nullmark:
            field_value = mark_safe(markdown.markdown(str(field_value))) if "html" in str(format).lower() else field_value
