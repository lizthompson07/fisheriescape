...
def ci_responsible_party(resource_person):
    ...
    # organisationName
    charstring(root,'gmd:organisationName',resource_person.person.organization.name_eng,resource_person.person.organization.name_fre)
    ...
    # civic
    charstring(ci_address,'gmd:deliveryPoint', resource_person.person.organization.address, resource_person.person.organization.address)
    #city
    charstring(ci_address,'gmd:city', resource_person.person.organization.city)
    #province
    charstring(ci_address,'gmd:administrativeArea', resource_person.person.organization.location.location_eng, resource_person.person.organization.location.location_fre)
    #postalcode
    charstring(ci_address,'gmd:postalCode', resource_person.person.organization.postal_code)
    #country
    charstring(ci_address,'gmd:country', resource_person.person.organization.location.country, resource_person.person.organization.location.country)
    #email
    charstring(ci_address,'gmd:electronicMailAddress', resource_person.person.user.email, resource_person.person.user.email)
    # role
    codelist(root, "gmd:role", "gmd:CI_RoleCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_90", resource_person.role.code,  resource_person.role.role)
    return root
...
class KeywordGroup:
    ...
    def create_group_element(self):
        ...
        # annotation #
        ##############
        if self.domain == 1: # NASA
            codelist(MD_Keywords, "gmd:type", "gmd:MD_KeywordTypeCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_101","RI_528", "theme; thème")
            thesaurusName = SubElement(MD_Keywords, 'gmd:thesaurusName')
            CI_Citation = SubElement(thesaurusName, 'gmd:CI_Citation')
            charstring(CI_Citation,'gmd:title',"NASA/Global Change Master Directory (GCMD) Science Keywords Version 6.0.0.0.0","NASA/Global Change Master Directory (GCMD) Science Keywords Version 6.0.0.0.0")
            date = SubElement(CI_Citation, 'gmd:date')
            CI_Date = SubElement(date, 'gmd:CI_Date')
            datestamp(CI_Date,'gmd:date',2007, 7, 23)
            codelist(CI_Date, "gmd:dateType", "gmd:CI_DateTypeCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_87","RI_368", "revision; révision")
            ...
            charstring(CI_ResponsibleParty,'gmd:individualName','LM Olsen, G Major, K Shein, J Scialdone, R Vogel, S Leicester, H Weir, S Ritz, T Stevens, M Meaux, C Solomon, R Bilodeau, M Holland, T Northcutt, RA Restrepo')
            ...
            codelist(CI_ResponsibleParty, "gmd:role", "gmd:CI_RoleCode", "http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#CI_RoleCode", 'owner',  'Owner')
            ...
            thesaurusName = SubElement(MD_Keywords, 'gmd:thesaurusName')
            CI_Citation = SubElement(thesaurusName, 'gmd:CI_Citation')
            charstring(CI_Citation,'gmd:title',"Integrated Taxonomic Information System (ITIS)","Integrated Taxonomic Information System (ITIS)")
            date = SubElement(CI_Citation, 'gmd:date')
            CI_Date = SubElement(date, 'gmd:CI_Date')
            datestamp(CI_Date,'gmd:date',2017, 10, 8)
            codelist(CI_Date, "gmd:dateType", "gmd:CI_DateTypeCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_87","RI_368", "revision; révision")
            citedResponsibleParty = SubElement(CI_Citation, 'gmd:citedResponsibleParty')
            CI_ResponsibleParty = SubElement(citedResponsibleParty, 'gmd:CI_ResponsibleParty')
            charstring(CI_ResponsibleParty,'gmd:organisationName','Integrated Taxonomic Information System (ITIS)', 'Integrated Taxonomic Information System (ITIS)')
            contactInfo = SubElement(CI_ResponsibleParty, 'gmd:contactInfo')
            CI_Contact = SubElement(contactInfo, 'gmd:CI_Contact')
            onlineResource = SubElement(CI_Contact, 'gmd:onlineResource')
            CI_OnlineResource = SubElement(onlineResource, 'gmd:CI_OnlineResource')
            ...
            date = SubElement(CI_Citation, 'gmd:date')
            CI_Date = SubElement(date, 'gmd:CI_Date')
            datestamp(CI_Date,'gmd:date',2018, 4, 26)
            codelist(CI_Date, "gmd:dateType", "gmd:CI_DateTypeCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_87","RI_368", "revision; révision")
            citedResponsibleParty = SubElement(CI_Citation, 'gmd:citedResponsibleParty')
            CI_ResponsibleParty = SubElement(citedResponsibleParty, 'gmd:CI_ResponsibleParty')
            charstring(CI_ResponsibleParty,'gmd:organisationName','U.S. National Library of Medicine', 'U.S. National Library of Medicine')
            contactInfo = SubElement(CI_ResponsibleParty, 'gmd:contactInfo')
            CI_Contact = SubElement(contactInfo, 'gmd:CI_Contact')
            onlineResource = SubElement(CI_Contact, 'gmd:onlineResource')
            CI_OnlineResource = SubElement(onlineResource, 'gmd:CI_OnlineResource')
            ...
            thesaurusName = SubElement(MD_Keywords, 'gmd:thesaurusName')
            CI_Citation = SubElement(thesaurusName, 'gmd:CI_Citation')
            charstring(CI_Citation,'gmd:title',"uncontrolled vocabulary; user-specified", "vocabulaire libre; spécifié par l'utilisateur")
            date = SubElement(CI_Citation, 'gmd:date')
            CI_Date = SubElement(date, 'gmd:CI_Date')
            datestamp(CI_Date,'gmd:date',2018, 4, 29)
            codelist(CI_Date, "gmd:dateType", "gmd:CI_DateTypeCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_87","RI_368", "revision; révision")
            citedResponsibleParty = SubElement(CI_Citation, 'gmd:citedResponsibleParty')
            CI_ResponsibleParty = SubElement(citedResponsibleParty, 'gmd:CI_ResponsibleParty')
            ...
            thesaurusName = SubElement(MD_Keywords, 'gmd:thesaurusName')
            CI_Citation = SubElement(thesaurusName, 'gmd:CI_Citation')
            charstring(CI_Citation,'gmd:title',"World Register of Marine Species (WoRMS)","World Register of Marine Species (WoRMS)")
            date = SubElement(CI_Citation, 'gmd:date')
            CI_Date = SubElement(date, 'gmd:CI_Date')
            ...
            citedResponsibleParty = SubElement(CI_Citation, 'gmd:citedResponsibleParty')
            CI_ResponsibleParty = SubElement(citedResponsibleParty, 'gmd:CI_ResponsibleParty')
            charstring(CI_ResponsibleParty,'gmd:organisationName','World Register of Marine Species', 'World Register of Marine Species')
            contactInfo = SubElement(CI_ResponsibleParty, 'gmd:contactInfo')
            CI_Contact = SubElement(contactInfo, 'gmd:CI_Contact')
            onlineResource = SubElement(CI_Contact, 'gmd:onlineResource')
            CI_OnlineResource = SubElement(onlineResource, 'gmd:CI_OnlineResource')
            ...
            thesaurusName = SubElement(MD_Keywords, 'gmd:thesaurusName')
            CI_Citation = SubElement(thesaurusName, 'gmd:CI_Citation')
            charstring(CI_Citation,'gmd:title',"Government of Canada Core Subject Thesaurus","Thésaurus des sujets de base du gouvernement du Canada")
            date = SubElement(CI_Citation, 'gmd:date')
            CI_Date = SubElement(date, 'gmd:CI_Date')
            datestamp(CI_Date,'gmd:date',2004)
            codelist(CI_Date, "gmd:dateType", "gmd:CI_DateTypeCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_87","RI_366", "creation; création")
            date = SubElement(CI_Citation, 'gmd:date')
            CI_Date = SubElement(date, 'gmd:CI_Date')
            ...
            citedResponsibleParty = SubElement(CI_Citation, 'gmd:citedResponsibleParty')
            CI_ResponsibleParty = SubElement(citedResponsibleParty, 'gmd:CI_ResponsibleParty')
            charstring(CI_ResponsibleParty,'gmd:organisationName','Government of Canada; Library and Archives Canada', 'Gouvernement du Canada; Bibliothèque et Archives Canada')
            contactInfo = SubElement(CI_ResponsibleParty, 'gmd:contactInfo')
            CI_Contact = SubElement(contactInfo, 'gmd:CI_Contact')
            onlineResource = SubElement(CI_Contact, 'gmd:onlineResource')
            CI_OnlineResource = SubElement(onlineResource, 'gmd:CI_OnlineResource')
            ...
            charstring(CI_OnlineResource,'gmd:protocol','WWW:LINK-1.0-http--link')
            codelist(CI_ResponsibleParty, "gmd:role", "gmd:CI_RoleCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_90", 'RI_409',  'custodian; conservateur')

        elif self.domain == 7: # DFO areas
            codelist(MD_Keywords, "gmd:type", "gmd:MD_KeywordTypeCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_101","RI_525", "place")
            thesaurusName = SubElement(MD_Keywords, 'gmd:thesaurusName')
            CI_Citation = SubElement(thesaurusName, 'gmd:CI_Citation')
            ...
            charstring(CI_ResponsibleParty,'gmd:organisationName','Government of Canada; Fisheries and Oceans Canada', 'Gouvernement du Canada; Pêches et Océans Canada')
            contactInfo = SubElement(CI_ResponsibleParty, 'gmd:contactInfo')
            CI_Contact = SubElement(contactInfo, 'gmd:CI_Contact')
            onlineResource = SubElement(CI_Contact, 'gmd:onlineResource')
            CI_OnlineResource = SubElement(onlineResource, 'gmd:CI_OnlineResource')
            ...
            url.text = "http://gcmd.nasa.gov/index.html"
            charstring(CI_OnlineResource,'gmd:protocol','WWW:LINK-1.0-http--link')
            codelist(CI_ResponsibleParty, "gmd:role", "gmd:CI_RoleCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_90", 'RI_413',  'originator; créateur')
...
def construct(my_resource):
    ...
    codelist(root, "gmd:characterSet", "gmd:MD_CharacterSetCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_95","RI_458", "utf8; utf8")
    ...
    charstring(root,'gmd:metadataStandardName',"North American Profile of ISO 19115:2003 - Geographic information - Metadata", "Profil nord-américain de la norme ISO 19115:2003 - Information géographique - Métadonnées")
    ...
    codelist(PT_Locale, "gmd:languageCode", "gmd:LanguageCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_116","fra", "French; Français")
    codelist(PT_Locale, "gmd:Country", "gmd:Country", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_117","CAN", "Canada; Canada")
    codelist(PT_Locale, "gmd:characterEncoding", "gmd:MD_CharacterSetCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_458","RI_458", "utf8; utf8")
    ...
    datestamp(CI_Date,'gmd:date',my_resource.time_start_year, my_resource.time_start_month, my_resource.time_start_day)
    codelist(CI_Date, "gmd:dateType", "gmd:CI_DateTypeCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_87","RI_366", "creation; création")
    ...
    if my_resource.fgp_publication_date == None: # only pass in a publication date, which would be the current time
        ...
        codelist(CI_Date, "gmd:dateType", "gmd:CI_DateTypeCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_87","RI_367", "publication; publication")

    else: # publication AND revision dates are added

        # publication date
        CI_Date = SubElement(SubElement(CI_Citation, 'gmd:date'), 'gmd:CI_Date')
        datestamp(CI_Date,'gmd:date',my_resource.fgp_publication_date.year, my_resource.fgp_publication_date.month, my_resource.fgp_publication_date.day)
        codelist(CI_Date, "gmd:dateType", "gmd:CI_DateTypeCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_87","RI_367", "publication; publication")
    ...
    # status
    codelist(MD_DataIdentification, "gmd:status", "gmd:MD_ProgressCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_106",my_resource.status.code, my_resource.status.label)

    # resource Maintenance
    MD_MaintenanceInformation = SubElement(SubElement(MD_DataIdentification, 'gmd:resourceMaintenance'), 'gmd:MD_MaintenanceInformation')
    codelist(MD_MaintenanceInformation, "gmd:maintenanceAndUpdateFrequency", "gmd:MD_MaintenanceFrequencyCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_102",my_resource.maintenance.code, my_resource.maintenance.code)
    ...
    MD_LegalConstraints = SubElement(SubElement(MD_DataIdentification, 'gmd:resourceConstraints'), 'gmd:MD_LegalConstraints')
    charstring(MD_LegalConstraints,'gmd:useLimitation', "Open Government Licence - Canada (http://open.canada.ca/en/open-government-licence-canada)", "Licence du gouvernement ouvert - Canada (http://ouvert.canada.ca/fr/licence-du-gouvernement-ouvert-canada)")
    codelist(MD_LegalConstraints, "gmd:accessConstraints", "gmd:MD_RestrictionCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_107","RI_606", "license; licence")
    codelist(MD_LegalConstraints, "gmd:useConstraints", "gmd:MD_RestrictionCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_107","RI_606", "license; licence")

    # General Constraints
    MD_Constraints = SubElement(SubElement(MD_DataIdentification, 'gmd:resourceConstraints'), 'gmd:MD_Constraints')
    charstring(MD_Constraints,'gmd:useLimitation', my_resource.resource_constraint_eng, my_resource.resource_constraint_fre )

    # Security Constraints
    MD_SecurityConstraints = SubElement(SubElement(MD_DataIdentification, 'gmd:resourceConstraints'), 'gmd:MD_SecurityConstraints')
    charstring(MD_SecurityConstraints,'gmd:useLimitation', my_resource.security_use_limitation_eng, my_resource.security_use_limitation_fre)
    ...
    # characterSet
    codelist(MD_DataIdentification, "gmd:characterSet", "gmd:MD_CharacterSetCode", "http://nap.geogratis.gc.ca/metadata/register/napMetadataRegister.xml#IC_195",my_resource.data_char_set.code, my_resource.data_char_set.label)
    ...
    beginPosition.text = date_converter(my_resource.time_start_year,my_resource.time_start_month,my_resource.time_start_day)
    if my_resource.time_end_year != None:
        endPosition.text = date_converter(my_resource.time_end_year,my_resource.time_end_month,my_resource.time_end_day)
    ...
    charstring(MD_DataIdentification,'gmd:supplementalInformation', suppl_info.create_english_block(), suppl_info.create_french_block())
...
def verify(resource):
    ...
    for keyword in resource.keywords.filter(~Q(keyword_domain_id=8)):
        if keyword.text_value_fre == None or keyword.text_value_fre == "":
            ...
            SubElement(checklist,'li').text ="English value for keyword is needed for '{}'".format(keyword.text_value_fre)
    ...
    # QC process
    if (resource.qc_process_descr_eng != None and resource.qc_process_descr_fre == None) or (resource.qc_process_descr_eng == None and resource.qc_process_descr_fre != None) :
        SubElement(checklist,'li').text = "'QC Process description' is an optional field, but if entered, must be present in both languages"

    # Physical Sample Description
    if (resource.physical_sample_descr_eng != None and resource.physical_sample_descr_fre == None) or (resource.physical_sample_descr_eng == None and resource.physical_sample_descr_fre != None) :
        SubElement(checklist,'li').text = "'Physical Sample Description' is an optional field, but if entered, must be present in both languages"

    # Parameters Collected
    if (resource.parameters_collected_eng != None and resource.parameters_collected_fre == None) or (resource.parameters_collected_eng == None and resource.parameters_collected_fre != None) :
        SubElement(checklist,'li').text = "'Parameters Collected' is an optional field, but if entered, must be present in both languages"

    # Sampling Method
    if (resource.sampling_method_eng != None and resource.sampling_method_fre == None) or (resource.sampling_method_eng == None and resource.sampling_method_fre != None) :
        SubElement(checklist,'li').text = "'Sampling Method' is an optional field, but if entered, must be present in both languages"

    # Resource Constraints
    if (resource.resource_constraint_eng != None and resource.resource_constraint_fre == None) or (resource.resource_constraint_eng == None and resource.resource_constraint_fre != None) :
        SubElement(checklist,'li').text = "'Resource Constraints' is an optional field, but if entered, must be present in both languages"


    return str(ElementTree.tostring(checklist, 'utf-8')).replace("b'",'').replace("&gt;",">").replace("&lt;","<").replace('b"','')
