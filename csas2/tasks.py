...
from . import emails
from .models import ToRReviewer, RequestReviewer
...
@shared_task(name="request_reviewer_reminder_email")
def request_reviewer_reminder_email():
    # get all reviewers whose status are pending (30) and whose decisions are null
    lazy_reviewers = RequestReviewer.objects.filter(review_started__isnull=False, review_completed__isnull=True)
    for r in lazy_reviewers:
        td = timezone.now() - r.review_started
        if td.days >= 5:
            send_email = False
            ...
            if send_email:
                # update the reminder sent date
                email = emails.RequestReviewReminderEmail(r, td)
                email.send()
                r.reminder_sent = timezone.now()
                r.save()
