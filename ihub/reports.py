...
def generate_summary_spreadsheet(fy, orgs):
    # figure out the filename
    target_dir = os.path.join(settings.BASE_DIR, 'media', 'ihub', 'temp')
    target_file = "temp_data_export_{}.xlsx".format(timezone.now().strftime("%Y-%m-%d"))
    target_file_path = os.path.join(target_dir, target_file)
    ...
    for org in org_list:
        if org.entries.count() > 0:
            ...
            for e in entry_list.filter(organizations=org):
                ...
                if e.organizations.count() > 0:
                    orgs = str([str(obj) for obj in e.organizations.all()]).replace("[", "").replace("]", "").replace("'", "").replace('"', "").replace(', ', "\n")
                else:
                    orgs = None

                if e.people.count() > 0:
                    people = str(["{} - {} ({})".format(obj.get_role_display(), obj, obj.organization) for obj in e.people.all()]).replace("[", "").replace("]", "").replace("'", "").replace('"', "").replace(', ', "\n")
                else:
                    people = None
                ...
                if e.sectors.count() > 0:
                    sectors = str([str(obj) for obj in e.sectors.all()]).replace("[", "").replace("]", "").replace("'", "").replace('"', "").replace(', ', "\n")
                else:
                    sectors = None
